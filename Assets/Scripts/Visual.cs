...
{//really need to rename this class and maybe also its internal structures because they are mad unclear.
    ...
    private List<AnimationController> myAnims;
    ...
    public void AttachModelsToThis()
    {
        if (vName.Contains("Anim"))
        {
            ...
            Debug.Log(vName + " -- ");
            myAnims = modelDictionary.AnimationsByAnimName[vName];
        }
        else
        { 
        GameObject modelDictionaryObject = GameObject.Find("ModelDictionaryObject");//change names christ
    ...
    public void ShowMe() {
        ...
        {
            animController = Instantiate(myAnims[0]);
            animController.transform.parent = this.transform;
        }
